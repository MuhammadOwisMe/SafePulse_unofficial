...
export default function TrustedContacts() {
  ...
  const BACKEND_URL = "http://192.168.100.111:3000";
        ...
        onPress: async () => {
          try {
            const response = await fetch(
              `${BACKEND_URL}/api/delete-contact/${id}`,
              {
                method: "DELETE",
              },
            );
  ...
  const sendRealTimeAlert = async (name: string, phone: string) => {
    try {
      ...
      const myName =
        userData?.fullName || user?.displayName || "SafePulse User";
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>
          SafePulse: {contacts.length}/3 Saved
        </Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <TouchableOpacity
          style={[
            ...
            contacts.length >= 3 &&
              !editingId && { backgroundColor: "#94a3b8" },
          ]}
          onPress={() =>
            contacts.length < 3
              ? setShowForm(true)
              : Alert.alert("Limit Reached", "Delete a contact first.")
          }
        >
            ...
            <View key={item.id} style={styles.contactCard}>
              ...
              <View style={styles.actionRow}>
                <TouchableOpacity
                  ...
                  onPress={() => sendRealTimeAlert(item.name, item.phone)}
                >
                ...
                <TouchableOpacity
                  style={[
                    styles.priorityBtn,
                    item.isPriority && styles.priorityActive,
                  ]}
                  onPress={() => togglePriority(item.id, item.isPriority)}
                >
                  <Star
                    size={16}
                    color={item.isPriority ? "#f59e0b" : "#64748b"}
                    fill={item.isPriority ? "#f59e0b" : "transparent"}
                  />
                  <Text
                    style={[
                      styles.priorityText,
                      item.isPriority && { color: "#f59e0b" },
                    ]}
                  >
                    Priority
                  </Text>
